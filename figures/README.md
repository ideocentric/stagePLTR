...
- **Now:** consolidated generator + placeholder parts; output renders correctly
  through stagePLTR's `QSvgRenderer` (CSS classes verified).
- **Next (step b):** generate.py emits a `catalog.json` fragment for the curated
  built-ins and **object packs** (`objects.json` + SVGs) using `footprint_units`
  for `viewBox`/`defaultSize`.
- **Then:** stagePLTR gains **Import Object Pack…** to load a pack into the user
  library (builds on Phase 3). The placeholder `parts/` get replaced with
  properly-scaled art (hand-authored or via the Blender pipeline).
